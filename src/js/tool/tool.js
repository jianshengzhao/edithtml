import tools from './tools' // 便捷工具
import librarys from './leftLibrary' // 左侧可操作组件库
import layer from './rightLayer' // 右侧图层

let tool = {
  init: function (self, $) { // 函数初始化
    // console.log(tools.RandomNumbers())
    let me = this
    me.$ = $
    me.doc = me.$(document)
    me.mod = me.$('.tl_mod')
    ...
    librarys.renderLibrary(self, me)   
  ...
  carryLineEvent: function (self, show) { // 计算参考线函数 wap todo:
    ...
    me.rowT.css('top', self.inp_y + canvasTop - scrollTop)
    me.rowB.css('top', self.inp_y + canvasTop + self.inp_h - scrollTop - 1)
    me.colL.css('left', self.inp_x + canvasLeft)
    me.colR.css('left', self.inp_x + canvasLeft + self.inp_w)
    if (!show) {
      me.line.show()
    }
  }, 
  ...
  carrySignMultiBoxEvent: function (self) { // 多选模块初始化
    let me = this
    ...
    if (onModule.length == 1 && !me.ctrl){
      me.$('.multiBox').remove()
      me.carrySignEvent(self, getElementY)
    } else if(onModule.length > 1 || me.ctrl){ 
  ...
  carryModuleOperationEvent: function (self, type, val) { // 属性栏模块操作事件集合 注：禁止了图片拉伸
    ...
    // -------------- css属性 -----------------
      ...
      case 'top':
        ...
        let moveY = val
        if (onModules.length > 1){
          maxY = self.inp_height - (parseInt(self.moduleElementB.css('top')) + parseInt(self.moduleElementB.css('height')) - parseInt(self.moduleElementY.css('top')))
        } else {          
          maxY = self.inp_height - self.inp_h
        }        
        if (val > maxY) {
          val = maxY
        }
  ...
  // ------------- 页头页尾调整 ----------------
    ...
    librarys.bindEvents(self, me) // 左边模块库
    layer.bindEvents(self, me) // 右侧图层栏
  }
}
